...
public class NetworkDefinition {
     ...
     */
    public void setFormatMode(String modeIn) {
        if (modeIn == null) {
            setType("isolated");
        }
        else {
    ...
    public static NetworkDefinition parse(String xml) {
        ...
        try {
            ...
            def.setFormatMode(forwardNode != null ? forwardNode.getAttributeValue("mode") : null);

            def.setName(Optional.ofNullable(netElement.getChildText("name")));
            def.setUuid(Optional.ofNullable(netElement.getChildText("uuid")));

            Map<String, BiConsumer<NetworkDefinition, Element>> forwardMapper = Map.of(
                    "nat", (d, node) -> d.setNat(Optional.of(NatDef.parseNat(node))),
                    "pf", (d, node) -> d.setPhysicalFunction(Optional.of(node.getAttributeValue("dev"))),
                    "interface", (d, node) -> d.interfaces.add(node.getAttributeValue("dev")),
                    "address", (d, node) -> d.virtualFunctions.add(NetworkDefinition.getPCIAddress(node))
            );
            if (forwardNode != null) {
                for (Object child : forwardNode.getChildren()) {
                    Element node = (Element) child;
                    if (forwardMapper.containsKey(node.getName())) {
                        forwardMapper.get(node.getName()).accept(def, node);
                    }
                }
            }
