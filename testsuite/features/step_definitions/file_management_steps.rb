...
When(/^I bootstrap "([^"]*)" using bootstrap script with activation key "([^"]*)" from the (server|proxy)$/) do |host, key, target_type|
  ...
  mgrctl = target.has_mgrctl ? 'mgrctl' : ''
  output, = target.run_local("sed -i '/^set timeout /c\\set timeout #{DEFAULT_TIMEOUT}' /tmp/#{boostrap_script} && expect -f /tmp/#{boostrap_script} #{system_name} #{mgrctl}", verbose: true)
